  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
      <div className="border-b border-gray-200 p-4">
        <h3 className="text-lg font-semibold text-gray-800">Replacement Actions</h3>
        <p className="text-sm text-gray-600">Define easy, healthier actions ready to replace anxious responses.</p>
      </div>
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
          ...
          <label className="block">
            <span className="text-sm text-gray-700">Reward text</span>
            <input
              type="text"
              value={rewardText}
          ...
          <label className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700">Mark as easy</span>
          </label>
          <button
            onClick={addAction}
        ...
        <ul className="space-y-2">
          {actions.length === 0 && (
            <li className="text-sm text-gray-600">No actions yet. Add your first replacement action.</li>
          )}
          {actions.map((a) => (
            <li key={a.id} className="p-3 bg-gray-50 rounded-md border border-gray-200 flex items-center justify-between">
              <div>
                <div className="font-medium text-gray-800">{a.title}</div>
                <div className="text-xs text-gray-600">Reward: {a.rewardText || '—'}</div>
              </div>
