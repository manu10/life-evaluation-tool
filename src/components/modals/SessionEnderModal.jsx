...
export default function SessionEnderModal({ isOpen, onClose, onEnd }) {
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center" role="dialog" aria-modal="true" aria-label="End Session">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Finish Session</h3>
          <button onClick={onClose} className="text-gray-600 hover:text-gray-800" aria-label="Close">✕</button>
        </div>
        <div className="space-y-4">
          <label className="block">
            <span className="text-sm text-gray-700">Enjoyment (1–5)</span>
            <input type="number" min={1} max={5} value={enjoyment} onChange={(e)=>setEnjoyment(parseInt(e.target.value || '0', 10))} className="mt-1 w-24 p-2 border border-gray-300 rounded-md" />
          </label>
          <label className="block">
            <span className="text-sm text-gray-700">One highlight (optional)</span>
            <input value={highlight} onChange={(e)=>setHighlight(e.target.value)} className="mt-1 w-full p-2 border border-gray-300 rounded-md" placeholder="What was interesting or useful?" />
          </label>
