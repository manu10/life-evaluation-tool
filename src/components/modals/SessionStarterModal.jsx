    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center" role="dialog" aria-modal="true" aria-label="Start Session">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Start Focus Session</h3>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2 mb-2">
              <button className={`px-2 py-1 text-xs rounded border ${hookMode==='pick'?'bg-blue-600 text-white border-blue-700':'bg-white text-blue-700 border-blue-300'}`} onClick={() => setHookMode('pick')}>Pick</button>
              <button className={`px-2 py-1 text-xs rounded border ${hookMode==='new'?'bg-blue-600 text-white border-blue-700':'bg-white text-blue-700 border-blue-300'}`} onClick={() => setHookMode('new')}>New</button>
            </div>
            {hookMode === 'pick' ? (
              <select value={hookId || ''} onChange={(e) => setHookId(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md">
                {hooks.length === 0 ? (
              ...
              <div className="flex items-center gap-2">
                <input value={newHookLabel} onChange={(e)=>setNewHookLabel(e.target.value)} placeholder="e.g., I am the kind of person who..." className="flex-1 p-2 border border-gray-300 rounded-md" />
          ...
          <div>
            <label className="block text-sm text-gray-700 mb-1">Quest title (optional)</label>
            <input value={questTitle} onChange={(e)=>setQuestTitle(e.target.value)} placeholder="Name this focus" className="w-full p-2 border border-gray-300 rounded-md" />
          </div>

          <div>
            <label className="block text-sm text-gray-700 mb-1">Duration (minutes)</label>
            <input type="number" min={5} max={180} value={minutes} onChange={(e)=>setMinutes(parseInt(e.target.value || '0', 10))} className="w-32 p-2 border border-gray-300 rounded-md" />
          </div>
        </div>
